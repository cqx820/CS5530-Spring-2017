...
public class TH {
	...
	public boolean checkupdate(int h_id, String login, Statement stmt) {
		boolean check = false;
		String sql = "select * from TH " + "where login ='" + login + "'" + " and "+" h_id =" + h_id;
	...
	// leave
	public ArrayList<String> geth_id(String login, Statement stmt) {
		...
		try {
			rs = stmt.executeQuery(sql);
			while (rs.next()) {
				if (rs.getString("h_id") == null || rs.getString("h_id").length() == 0
						|| rs.getString("h_id").equals(" ")) {
					break;
				}
				resultList.add(rs.getString("h_id")+"\t"+rs.getString("category")+"\t"+rs.getString("address")+
						"\t"+rs.getString("price")+"\t"+rs.getString("name")+"\t"+rs.getString("telephone")+"\t"+rs.getString("keyword")+"\t"+rs.getString("yearBuilt")
						+"\t"+rs.getString("url")+"\t"+rs.getString("City")+"\t"+rs.getString("State"));
			}
			rs.close();
		} catch (Exception e) {
			System.err.println(e.getMessage());
	...
	public int getLatestReserve(String login, Statement stmt) {
		
		int hid= -1;
		String sql = "select r.h_id from Reserve r " + "where r.login= '" + login
				+ "' ORDER BY r.reserve_date DESC LIMIT 1";
		ResultSet rs = null;
		try {
			rs = stmt.executeQuery(sql);
			// System.out.println(sql);
